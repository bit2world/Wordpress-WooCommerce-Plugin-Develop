...
class Shortcodes {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style(
			'wppic-style-custom',
			Functions::get_plugin_url( 'dist/wppic-custom.css' ),
			array(),
			Functions::get_plugin_version(),
			'all'
		);
	...
	public static function shortcode_function( $atts, $content = '' ) {
		...
		$attributes = shortcode_atts(
			array(
				...
				'col_gap'     => 20,
				'row_gap'     => 20,
				'myimage'     => '',  // image url to replace WP logo (175px X 175px).
		...
		if ( is_array( $slug ) && $multi ) {
			...
			foreach ( $slug as $asset_slug ) {
				...
				if ( ! empty( $custom ) ) {
					...
					if ( 'yes' === $ajax ) {
						...
						$ajax_data   = 'data-type="' . $type . '" data-slug="' . $asset_slug . '" data-image="' . $image . '" data-expiration="' . $expiration . '"  data-layout="' . $layout . '" '. '" data-myimage="' . $myimage. '" ' ;
					}
					...
					if ( 'yes' !== $ajax ) {
						$content .= self::shortcode_content( $type, $asset_slug, $image, $expiration, $layout , $myimage);
					} else {
			...
			if ( ! empty( $custom ) ) {
				...
				if ( 'yes' === $ajax ) {
					$add_class[] = 'wp-pic-ajax';
					$ajax_data   = 'data-type="' . $type . '" data-slug="' . $slug . '" data-image="' . $image. '" data-expiration="' . $expiration . '"  data-layout="' . $layout . '" '.'" data-myimage="' . $myimage . '" ';
				}
				...
				if ( 'yes' !== $ajax ) {
					$content .= self::shortcode_content( $type, $slug, $image, $expiration, $layout , $myimage);
				} else {
					$content .= '<div class="wp-pic-body-loading"><div class="signal"></div></div>';
				}
	 ...
	 */
	public static function shortcode_content( $type = null, $slug = null, $image = null, $expiration = null, $layout = null,  $myimage = null ) {
		...
		$content = '';
		$content = apply_filters( 'wppic_add_template', $content, array( $type, $wppic_data, $image, $layout , $myimage) );
